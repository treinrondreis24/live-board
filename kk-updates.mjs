...
export async function updatePage({before='',teams=[]}={}){
 ...
 const rows=await kkUpdateRows(before,teams);const available=new Map();for(const p of await kkList('participant')){const id=members.get(p.id)||'solo-'+p.id;available.set(id,names.get(id)||p.value.displayName||'Deelnemer');}
 const mapped=rows.map(r=>{const teamId=members.get(r.owner)||'solo-'+r.owner,teamName=names.get(teamId)||r.value.displayName||'Deelnemer';available.set(teamId,teamName);return {id:r.id,text:r.value.text,displayName:r.value.displayName,createdAt:Number(r.created),media:r.value.media,teamId,teamName};});
 const filtered=mapped;
 const updates=filtered.slice(0,30),last=updates.at(-1);
 return {updates,teams:[...available].map(([id,name])=>({id,name})).sort((a,b)=>a.name.localeCompare(b.name)),next:filtered.length>30?`${String(last.createdAt).padStart(16,'0')}:${last.id}`:null};
}
import {kkUpdateRows} from './kk-store.mjs';
export async function updateMedia(updateId,mediaId){
 ...
 return {url:await authorizedMediaUrl(media.owner,mediaId),type:media.value.type};
}
