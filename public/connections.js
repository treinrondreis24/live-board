import {boardRows,shortCity,destinationCity,statusView,eventDelay} from './connections-view.js';
...
function journey(e,mode){const t=e.planned,delay=eventDelay(e);return `<div class="journey"><div class="journey-line"><time>${time(t)}</time>${delay&&!e.cancelled?`<span class="time-delay ${delay>=30?'late':''}">${delay>0?'+':''}${delay}</span>`:''}<span class="city" title="${esc(mode==='arrival'?e.origin:e.destination)}">${esc(destinationCity(mode==='arrival'?e.origin:e.destination)||'—')}</span></div><small>${t!==e.planned?`<del>${time(e.planned)}</del>`:''}${e.cancelled?'<span>Geannuleerd</span>':''}</small></div>`;}
function render(){
 const all=boardRows(data?.connections),total=1;page%=total;
 if(singleScreen&&window.parent!==window)window.parent.postMessage({type:'connection-board-count',count:all.length},location.origin);
