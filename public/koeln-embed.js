...
async function refresh(){
  try{
    ...
    if(!data){
      ...
      allTab.hidden=!(["NL","BE","NO","IT","FR","ES","SE"].includes(p.country)||p.source?.includes("OJP"));
      if((["NL","BE","NO","IT","FR","ES","SE"].includes(p.country)||p.source?.includes("OJP")))activeTab="all";
    ...
    if(p.country==="BE"&&!document.getElementById("belgium-source")){const a=document.createElement("a");a.id="belgium-source";a.href="https://data.belgianmobility.io/";a.textContent="Bron: NMBS — Belgian Mobility Company";document.querySelector('.board-sources').appendChild(a);}
    if(p.country==="NO"&&!document.getElementById("entur-source")){const a=document.createElement("a");a.id="entur-source";a.href="https://entur.no/";a.textContent="Bron: Entur (NLOD)";document.querySelector('.board-sources').appendChild(a);}
    if(p.source?.includes("OJP")&&!document.getElementById("swiss-source")){const a=document.createElement("a");a.id="swiss-source";a.href="https://opentransportdata.swiss/";a.textContent="Bron: opentransportdata.swiss (OJP)";document.querySelector('.board-sources').appendChild(a);}
    if(p.source?.includes('RFI')&&!document.getElementById('rfi-source')){const a=document.createElement('a');a.id='rfi-source';a.href='https://iechub.rfi.it/ArriviPartenze/ArrivalsDepartures/Home';a.textContent='Bron: RFI Monitor Arrivi/Partenze';document.querySelector('.board-sources').appendChild(a);}
    if(p.source?.includes('SNCF')&&!document.getElementById('sncf-source')){const a=document.createElement('a');a.id='sncf-source';a.href='https://transport.data.gouv.fr/datasets/horaires-sncf';a.textContent='Bron: SNCF Voyageurs (ODbL)';document.querySelector('.board-sources').appendChild(a);}
    for(const [source,label,url] of [['TRAFIKVERKET','Trafikverket (CC0)','https://data.trafikverket.se/'],['RENFE','Renfe (CC BY 4.0)','https://data.renfe.com/'],['EUROSTAR','Eurostar (Licence Ouverte 2.0)','https://www.data.gouv.fr/datasets/eurostar-gtfs-plan-de-transport-et-temps-reel']]){if(p.source?.includes(source)&&!document.getElementById(source+'-source')){const a=document.createElement('a');a.id=source+'-source';a.href=url;a.textContent='Bron: '+label;document.querySelector('.board-sources').appendChild(a);}}
    const sources=document.querySelector('.board-sources');if(!sources.children.length&&p.source){const label=document.createElement('span');label.textContent='Bron: '+p.source;sources.appendChild(label);}
    let notice=document.getElementById('station-notice');if(p.notice&&!notice){notice=document.createElement('p');notice.id='station-notice';document.querySelector('.board').insertBefore(notice,document.querySelector('.board-sources'));}if(notice){notice.textContent=p.notice||'';notice.hidden=!p.notice;}
    ...
    scanStatus.textContent=d&&!Number.isNaN(d.getTime())
      ...
      :"actueel";
    if((["BE","NO","IT","FR","ES","SE"].includes(p.country)||p.source?.includes("OJP"))&&p.status!=="ready")scanStatus.textContent=p.status==="starting"?"dienstregeling laden…":"realtime tijdelijk niet beschikbaar";
  }catch(e){
...
function applyAppearance(p){
 ...
 document.body.dataset.boardDesign=['db','trenitalia','uk','sncf','treinreiziger'].includes(a.design)?a.design:'standard';
 ...
 let foot=document.getElementById('custom-footer');if(!foot){foot=document.createElement('p');foot.id='custom-footer';board.insertBefore(foot,document.querySelector('.board-sources'));}foot.textContent=settings.footer||'';foot.hidden=!settings.footer;
