...
export function seal(value){const iv=randomBytes(12),c=createCipheriv('aes-256-gcm',key(),iv);return Buffer.concat([iv,c.update(value,'utf8'),c.final(),c.getAuthTag()]).toString('base64url');}
...
export function consumeTotp(user,code,now){if(!/^\d{6}$/.test(String(code)))return false;const current=Math.floor(now/30000);for(const step of [current,current-1,current+1])if(step>(user.lastStep??-1)&&safe(totp(unseal(user.secret),step*30000),code)){user.lastStep=step;return true;}return false;}
export async function passwordHash(p){const salt=randomBytes(16).toString('hex');return salt+':'+(await scrypt(p,salt,64,{N:32768,r:8,p:1,maxmem:64*1024*1024})).toString('hex');}
