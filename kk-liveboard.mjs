...
export function liveRow(row,claim,now){
 ...
 return {year:Number(p.competitionYear||p.competition||2026),lastProof:row.lastProof||null,id:p.id,name:p.fullName||p.displayName||p.email,edition:p.edition,startStation:p.station||'',endStation:claim?.endStation||p.withdrawal?.endStation||'',startTime:p.startTime,startDate:p.startDate||'2026-09-19',start,
