...
const key=(train,date,ref='',source='OJP')=>ref?source+'|'+date+'|'+ref:'NDOV|'+train+'|'+date;
...
export function applyJourneySnapshot(previous,snapshot,receivedAt=Date.now()){
 ...
 state.journeyId=key(snapshot.trainNumber,snapshot.serviceDate,snapshot.journeyRef,snapshot.archiveSource);state.archiveSource=snapshot.journeyRef?(snapshot.archiveSource||'OJP'):'NDOV';state.journeyRef=snapshot.journeyRef||state.journeyRef;
...
export async function recordJourneyMeasurements(trains,source,at){
 ...
 for(const number of numbers){
  ...
  for(const row of rows){const state=JSON.parse(row.state_json);if(!state)continue;const stops=[];
   ...
   if(stops.length)await recordJourneySnapshot({trainNumber:number,serviceDate:state.serviceDate,journeyRef:state.journeyRef,archiveSource:state.archiveSource,source:'DB',sourceTimestamp:at,category:state.category,completePlan:false,stops});
  }
 }
}
...
async function saveSnapshot(snapshot){
 ...
 const id=key(snapshot.trainNumber,snapshot.serviceDate,snapshot.journeyRef,snapshot.archiveSource),now=Date.now(),revisionId=hash([id,snapshot]);
 if(backend==='sqlite'){
  sqlite.exec('BEGIN');
  try{
