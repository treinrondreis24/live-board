...
export function applyJourneySnapshot(previous,snapshot,receivedAt=Date.now()){
 ...
 for(const incoming of snapshot.stops){
   ...
   for(const mode of ['arrival','departure']){
     ...
     if(input.measurementOnly&&(!event.measurement||sourceTimestamp>=event.measurement.sourceTimestamp))event.measurement={expectedTime:input.expectedTime??null,platform:input.currentPlatform??null,cancelled:Boolean(input.cancelled),...meta};
 ...
 for(const number of numbers){
  ...
  for(const row of rows){const state=JSON.parse(row.state_json);if(!state)continue;const stops=[];
   for(const t of trains.filter(t=>String(t.number)===number&&(t.hasRealtime===true||t.cancelled))){
    ...
    const matches=state.stops.filter(s=>stationMatch(s.station)===stationMatch(t.observedAt)&&s[mode]&&[s[mode].original?.time,s[mode].planning?.time].some(time=>Number(time)===Number(t.plannedTimestamp)));
    if(matches.length!==1)continue;const stop=matches[0];stops.push({id:stop.id,station:stop.station,stationCode:stop.stationCode,sequence:stop.sequence,[mode]:{measurementOnly:true,expectedTime:t.expectedTimestamp,currentPlatform:t.currentTrack||null,cancelled:t.cancelled}});
   }
...
export async function getJourney(train,date,journeyId){
 ...
 const state=row?JSON.parse(row.state_json):null;return state&&state.trainNumber===train&&state.serviceDate===date?state:null;
}
