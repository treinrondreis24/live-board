...
vm.runInContext(app.slice(app.indexOf('function withinTrainWindow'),app.indexOf('function dbScreenDuration')),c);
...
assert.equal(vm.runInContext('screenStatus({delay:30})',c),'+30');assert(vm.runInContext('screenStatus({delay:0,hasRealtime:true})',c).includes('✓'));const planned=vm.runInContext('screenStatus({delay:0,hasRealtime:false})',c);assert.match(planned,/<svg[^>]+class="planning-clock"/);assert.match(planned,/aria-label="Gepland"/);
