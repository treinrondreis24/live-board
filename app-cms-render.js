(()=>{
 ...
 async function fetchFeed(url){const old=feeds.get(url);if(old&&Date.now()-old.at<300000)return old.data;const r=await fetch('/api/app-content/feed?'+new URLSearchParams({url,...(preview?{preview:'1'}:{})}));const j=await r.json();if(!r.ok)throw Error(j.error);feeds.set(url,{at:Date.now(),data:j});return j;}
