...
export async function handleTreinhuisAccess(req,res,url){
 ...
 if(owner){
 const name=String(data.name||'').trim();if(data.action==='revoke'){if(!s.members[name]){json(404,{error:'Gebruiker niet gevonden.'});return true;}s.members[name].active=false;delete s.members[name].resetHash;delete s.members[name].resetExpires;delete s.members[name].inviteHash;delete s.members[name].enrollment;for(const group of [s.sessions,s.devices])for(const [id,v] of Object.entries(group))if(v.username===name)delete group[id];await writeAdminSecurity(s,record.revision);json(200,{ok:true});return true;}
 ...
 Object.assign(user,{password:user.enrollment.password,secret:user.enrollment.secret,lastStep:user.enrollment.lastStep,recovery:[],active:true});delete user.inviteHash;delete user.inviteExpires;delete user.enrollment;delete user.resetHash;delete user.resetExpires;newSession(s,res,now,false,country,user.name);await writeAdminSecurity(s,record.revision);json(200,{ok:true});return true;
