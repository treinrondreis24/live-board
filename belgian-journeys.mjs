...
export function belgianIcePlans(read,stops,routes,trips,toTime,at){
 const explicit=t=>['311','313'].includes(String(t.trip_short_name).trim())&&/^(TRN|ICE)$/i.test(routes.get(t.route_id)?.route_short_name||'');
 const selected=new Map([...trips].filter(([,t])=>isBelgianIceRoute(routes.get(t.route_id))||explicit(t)).map(([id,t])=>[id,{trip:t,calls:[]}]));
