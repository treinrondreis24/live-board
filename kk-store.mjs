...
export async function initKKStore(connection){
 ...
 const testSQL=sql.replaceAll('kk_records','kk_test_records').replaceAll('kk_owner','kk_test_owner').replaceAll('kk_expiry','kk_test_expiry');
 const configSQL='CREATE TABLE IF NOT EXISTS kk_test_settings (id TEXT PRIMARY KEY,payload TEXT NOT NULL)';
 if(db.pool){await db.pool.query(sql);await db.pool.query(testSQL);await db.pool.query(configSQL);}else{db.sqlite.exec(sql);db.sqlite.exec(testSQL);db.sqlite.exec(configSQL);}
}
async function query(sql,args=[]){if(testEdition())sql=sql.replace(/\bkk_records\b/g,'kk_test_records');if(db.pool)return (await db.pool.query(sql,args)).rows;const values=[];const sqliteSQL=sql.replace(/\$(\d+)/g,(_,n)=>{values.push(args[Number(n)-1]);return '?';});return db.sqlite.prepare(sqliteSQL).all(...values);}
export async function kkTestConfig(){
 let [row]=await query("SELECT payload FROM kk_test_settings WHERE id='test-2026'");
 if(!row){
  const draft=await kkGet('hilta-next','draft');if(!draft)throw Object.assign(Error('Het testnetwerk is nog niet klaargezet.'),{status:503});
  const network=draft.value,hash=createHash('sha256').update(JSON.stringify(network)).digest('hex');
  const config={id:'test-2026',competitionYear:2026,label:'TESTEDITIE · regels 2026',network,hash,createdAt:Date.now()};
  await query("INSERT INTO kk_test_settings(id,payload) VALUES('test-2026',$1) ON CONFLICT(id) DO NOTHING",[JSON.stringify(config)]);
  [row]=await query("SELECT payload FROM kk_test_settings WHERE id='test-2026'");
 }
...
export async function kkPut(kind,id,owner,value,expires=0){value=editionValue(kind,value);await query('INSERT INTO kk_records(kind,id,owner,created,expires,payload) VALUES($1,$2,$3,$4,$5,$6) ON CONFLICT(kind,id) DO UPDATE SET payload=excluded.payload,expires=excluded.expires RETURNING id',[kind,id,owner,Date.now(),expires,JSON.stringify(value)]);}
export async function kkDelete(kind,id){await query('DELETE FROM kk_records WHERE kind=$1 AND id=$2 RETURNING id',[kind,id]);}
export async function kkInsert(kind,id,owner,value){value=editionValue(kind,value);return (await query('INSERT INTO kk_records(kind,id,owner,created,expires,payload) VALUES($1,$2,$3,$4,0,$5) ON CONFLICT(kind,id) DO NOTHING RETURNING id',[kind,id,owner,Date.now(),JSON.stringify(value)])).length===1;}
...
export async function kkPatchParticipant(id,changes){if(testEdition()){const row=await kkGet('participant',id);changes=editionValue('participant',{...row?.value,...changes});}const merge=db.pool?"(payload::jsonb || $2::jsonb)::text":"json_patch(payload,$2)";const [row]=await query("UPDATE kk_records SET payload="+merge+" WHERE kind='participant' AND id=$1 RETURNING payload",[id,JSON.stringify(changes)]);return row?JSON.parse(row.payload):null;}
...
export async function kkScoreRows(owner=null){const field=(a,k)=>db.pool?a+".payload::jsonb->>'"+k+"'":"json_extract("+a+".payload,'$."+k+"')";const rows=await query("SELECT p.payload, MAX(s.created) AS last_proof, COUNT(s.id) AS proofs, SUM(CASE WHEN "+field('h','status')+" IN ('participant-confirmed','admin-confirmed') THEN CAST("+field('h','km')+" AS REAL) ELSE 0 END) AS km, SUM(CASE WHEN s.id IS NOT NULL AND COALESCE("+field('h','status')+",'') NOT IN ('participant-confirmed','admin-confirmed') THEN 1 ELSE 0 END) AS pending FROM kk_records p LEFT JOIN kk_records s ON s.kind='submission' AND s.owner=p.id AND "+field('s','kind')+"='proof' LEFT JOIN kk_records h ON h.kind='hilta-current' AND h.id=s.id WHERE p.kind='participant'"+(owner?" AND p.id=$1":"")+" GROUP BY p.id,p.payload",owner?[owner]:[]);return rows.map(r=>({participant:JSON.parse(r.payload),lastProof:r.last_proof==null?null:Number(r.last_proof),proofs:Number(r.proofs),km:Math.round(Number(r.km||0)*10)/10,pending:Number(r.pending||0)}));}
...
export async function kkPendingRouteRows(){const rows=await query("SELECT s.id,s.owner,s.created,s.payload AS proof,p.payload AS participant,h.payload AS route FROM kk_records s JOIN kk_records p ON p.kind='participant' AND p.id=s.owner LEFT JOIN kk_records h ON h.kind='hilta-current' AND h.id=s.id AND h.owner=s.owner WHERE s.kind='submission' ORDER BY s.owner,s.created,s.id");return rows.map(r=>({...r,created:Number(r.created),proof:JSON.parse(r.proof),participant:JSON.parse(r.participant),route:r.route?JSON.parse(r.route):null})).filter(r=>r.proof.kind==='proof'&&!r.participant.deleting);}
export async function kkAdminRouteReplace(id,owner,value,expected){return (await query("INSERT INTO kk_records(kind,id,owner,created,expires,payload) VALUES('hilta-current',$1,$2,$3,0,$4) ON CONFLICT(kind,id) DO UPDATE SET payload=excluded.payload WHERE kk_records.owner=$2 AND kk_records.payload=$5 RETURNING id",[id,owner,Date.now(),JSON.stringify(value),expected])).length>0;}
export async function kkComparePut(kind,id,value,expected){return (await query('INSERT INTO kk_records(kind,id,owner,created,expires,payload) VALUES($1,$2,$1,$3,0,$4) ON CONFLICT(kind,id) DO UPDATE SET payload=excluded.payload WHERE kk_records.payload=$5 RETURNING id',[kind,id,Date.now(),JSON.stringify(value),expected])).length>0;}

// Bulk score input: avoids a database query per participant and never caps evidence at 200.
export async function kkScoringProofs(owner=null){
 const rows=await query("SELECT s.id,s.owner,s.created,s.payload,h.payload AS route FROM kk_records s LEFT JOIN kk_records h ON h.kind='hilta-current' AND h.id=s.id AND h.owner=s.owner WHERE s.kind='submission'"+(owner?" AND s.owner=$1":"")+" ORDER BY s.created,s.id",owner?[owner]:[]);
 const grouped=new Map();for(const r of rows){const proof=JSON.parse(r.payload);if(proof.kind!=='proof')continue;if(!grouped.has(r.owner))grouped.set(r.owner,[]);grouped.get(r.owner).push({id:r.id,created:Number(r.created),proof,route:r.route?JSON.parse(r.route):null});}return grouped;
}

function editionValue(kind,value){if(!testEdition()||kind!=='participant')return value;return {...value,editionId:'test-2026',competitionYear:2026,scoringVersion:`2026-${Number(value.edition)||24}-v1`,networkVersion:testEdition().network.revision};}
