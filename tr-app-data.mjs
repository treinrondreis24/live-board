...
export function parseNews(xml){
 ...
 return (Array.isArray(items)?items:[items]).slice(0,30).map(i=>({title:clean(i.title),url:safeUrl(i.link),date:i.pubDate,summary:clean(i.description).slice(0,240)})).filter(i=>i.url&&new URL(i.url).hostname.endsWith('treinreiziger.nl'));
}
...
export function createTreinreizigerHandler({stations,getBoard}){
 ...
 if(assets[url.pathname]){const file=assets[url.pathname],type=file.endsWith('.js')?'text/javascript':file.endsWith('.css')?'text/css':file.endsWith('.svg')?'image/svg+xml':file.endsWith('.webmanifest')?'application/manifest+json':'text/html';res.writeHead(200,{'Content-Type':type+'; charset=utf-8','Cache-Control':'no-cache','X-Content-Type-Options':'nosniff','Content-Security-Policy':"default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'self'; frame-ancestors 'self'; base-uri 'self'; form-action 'self'"});res.end(await fs.readFile(new URL(file,import.meta.url)));return true;}
