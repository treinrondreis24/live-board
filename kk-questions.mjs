...
export async function saveArticle(data){const order=Number(data.order||0);if(!Number.isInteger(order)||order<0||order>9999)fail('Gebruik een volgordenummer tussen 0 en 9999.');const id=data.id?key(data.id):randomUUID();if(!['summary','detail','faq'].includes(data.category)||![0,12,24].includes(Number(data.edition)))fail('Kies een geldig onderdeel en editie.');const article={id,title:txt(data.title,160),body:txt(data.body,10000),category:data.category,edition:Number(data.edition),year:2026,order,published:data.published===true,at:Date.now()};await kkPut('help-content',id,'content',article);return article;}
