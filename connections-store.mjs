...
export async function readConnectionRevisions(key){return (await query('SELECT revision,recorded_at,payload FROM connection_revisions WHERE connection_key=$1 ORDER BY revision DESC LIMIT 200',[key])).map(r=>({revision:Number(r.revision),recordedAt:Number(r.recorded_at),...JSON.parse(r.payload)}));}
