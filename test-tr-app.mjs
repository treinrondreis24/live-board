...
assert(ctx.check({category:'ICE',to:'Düsseldorf Hbf',route:[]},[{types:'ICE',direction:'Köln; Dusseldorf; Frankfurt'}]));assert(!ctx.check({category:'IC',to:'Frankfurt',route:[]},[{types:'ICE',direction:'Köln; Frankfurt'}]));
assert(catalog.some(s=>s.id==='nl-ut'&&s.name==='Utrecht Centraal'));
for(const [stored,count]of [[null,1],['[]',0],[JSON.stringify([{id:'existing'}]),1]]){let written;const seed={key:'test',favorites:[],localStorage:{getItem:()=>stored,setItem:(k,v)=>written=v}};vm.createContext(seed);vm.runInContext(js.slice(js.indexOf('function defaultStation('),js.indexOf('const esc=')),seed);assert.equal(seed.favorites.length,count);if(stored===null){assert.equal(seed.favorites[0].filters.length,7);assert(written);}else assert.equal(written,undefined);}
