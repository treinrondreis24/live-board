...
export const swissStations=Object.fromEntries([
  ['chur','Chur','9000'],['basel-sbb','Basel SBB','10'],['zuerich','Zürich HB','3000'],
...
export function swissRows(page,xml,now=Date.now()){
  ...
  const rows=list(data.StopEventResult).flatMap(result=>{
    ...
    if(!c||service?.Mode?.PtMode!=='rail')return [];
    // A platform may change, so identity uses the station and call order, not the quay.
    const stopId=[s.id,...s.extraStopIds||[]].find(id=>c.StopPointRef===id||String(c.StopPointRef||'').startsWith(id+':'));
    if(!stopId)return [];
    ...
    return [{id,source:'OJP',sourceTripId:journey,sourceEventId:id,serviceDate,trainKey:serviceDate+'|'+journey,number,train:[label,number].filter(Boolean).join(' '),category,line:label,transportSubmode:service.Mode.RailSubmode,
      ...
      from:text(service.OriginText)||s.name,to:text(service.DestinationText)||'—',route:[],messageTimestamp:sourceAt,rawStop:{call:c,service}}];
  });
...
export async function scanSwissStation(page,{fetcher=fetch,store=recordObservations,now=()=>Date.now(),key=process.env.OJP_API_KEY}={}){
  ...
  try{
    const results=[];
    for(const stopId of [swissStations[page].id,...swissStations[page].extraStopIds||[]]){
    ...
    try{await store(rows,'OJP',at);state.lastStoredAt=new Date(at).toISOString();}catch{state.status='storage-error';state.error='Opslag OJP mislukt';}
